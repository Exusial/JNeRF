...
class LinearLog(jt.nn.Optimizer):
    def __init__(self,nested_optimizer:jt.nn.Optimizer, start_lr=5e-4, end_lr=5e-6, max_steps=40000, lr_delay_steps=0,lr_delay_mult=1):
    ...
    def step(self, loss=None):
        if self.lr_delay_steps > 0:
            ...
            delay_rate = self.lr_delay_mult + (1 - self.lr_delay_mult) * jt.sin(0.5 * jt.array(np.pi) * jt.safe_clip(self.steps / self.lr_delay_steps, 0, 1))
        else:
            delay_rate = 1.
